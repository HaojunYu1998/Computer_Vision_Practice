...
def extract_position_embedding(position_mat, emb_dim, device, wave_length=1000):
    """
    ...
    # (emb_dim/8,)
    feat_range = torch.arange(emb_dim // 8).to(device)
    # (batch_images, num_boxes, num_boxes, 4) / (1, 1, 1, 1, emb_dim/8) 
    # => (batch_images, num_boxes, num_boxes, 4, emb_dim/8)
    dim_mat = torch.Tensor([wave_length]).to(device).pow((8./emb_dim) * feat_range).view(1, 1, 1, 1, -1)
    # (batch_images, num_boxes, num_boxes, 4, emb_dim/8)
...
def extract_position_matrix(proposal_boxes, device):
    """
    ...
    # (batch_images, num_boxes, num_boxes)
    pos1 = torch.log(torch.max(
        x_delta.abs(), torch.full(x_delta.shape, eps, dtype=torch.float32).to(device))
    ).unsqueeze(3)
    pos2 = torch.log(torch.max(
        y_delta.abs(), torch.full(x_delta.shape, eps, dtype=torch.float32).to(device))
    ).unsqueeze(3)
...
def extract_rank_embedding(rank_dim, feat_dim, device, wave_length=1000):
    """ 
    Args:
    ...
    """
    rank_range = torch.arange(0, rank_dim).to(device)
    feat_range = torch.arange(0, feat_dim / 2).to(device)
    dim_mat = torch.full((1,), wave_length, dtype=torch.float32).to(device).pow((2. / feat_dim) * feat_range).view(1, -1)
    rank_mat = rank_range[:,None,...].to(device)
...
def extract_pairwise_multi_position_embedding(position_mat, emb_dim, device, wave_length=1000):
    """
    Extract the position embedding from the position matrix
    Args:
    ...
    feat_range = torch.arange(emb_dim // 8).to(device)
    # (1, 1, 1, 1, 1, emb_dim / 8)
    dim_mat = torch.Tensor([wave_length]).to(device).pow((8./emb_dim) * feat_range).view(1, 1, 1, 1, 1, -1)
...
def extract_multi_position_matrix(boxes, device):
    ...
    # (batch_images, num_classes, num_boxes, num_boxes)
    pos1 = torch.log(torch.max(x_delta.abs(), torch.full(x_delta.shape, eps, dtype=torch.float32).to(device))).unsqueeze(4)
    pos2 = torch.log(torch.max(y_delta.abs(), torch.full(y_delta.shape, eps, dtype=torch.float32).to(device))).unsqueeze(4)
